...
bgimg = Image.open('waterfall.jpg')
l = tk.Label(root)
...
tk.Label(root, text="NHSQC Peaklist").grid(row=1)
tk.Label(root, text="HNCA  Peaklist").grid(row=2)
tk.Label(root, text="HNCACB  Peaklist").grid(row=3)
tk.Label(root, text="HNCO  Peaklist").grid(row=4)
tk.Label(root, text="Save BMRB Filename").grid(row=5)
tk.Label(root, text="Save Peaklist Filename").grid(row=6)
tk.Label(root, text="Please type in what residue number the first amino acid in the sequence file is\nI.E. if the first amino acid in the sequence file is 20, type in 20\n Click enter when done").grid(row=7)
tk.Label(root, text="Set RMSD Threshold (Recommended 2-3). Click enter when done").grid(row=8)
tk.Label(root, text="NMR_STAR V2 or V3").grid(row=9)
...
def NHSQC():
    ...
    label4=Label(root,text=fullpath).grid(row=1,column=1)
...
def HNCA():
    ...
    label5=Label(root,text=fullpath).grid(row=2,column=1)
...
def HNCACB():
    ...
    label6=Label(root,text=fullpath).grid(row=3,column=1)
...
def HNCO():
    ...
    label7=Label(root,text=fullpath).grid(row=4,column=1)
...
def save_file():
    ...
    label2=Label(root,text=fullpath).grid(row=5,column=1)
...
def save_file2():
    ...
    label8=Label(root,text=fullpath).grid(row=6,column=1)
...
def help():
    webbrowser.open('https://github.com/sam-mahdi/Peaklist_Assignment_Library-PAL-/blob/master/AVB/HELP/AVB_Manual.md')
...
def nmrstar():
    ...
    label8=Label(root,text=fullpath).grid(row=9,column=1)
...
def sparta_gen_only():
    ...
    if save_file_sparta == ():
        text_area.insert(tk.INSERT,'please indicate bmrb save file (make sure to use browse)\n')
    if seq_start == ():
...
#Since prolines lack the amide nitrogen and hydrogen, they are added in
        ...
        url='http://www.bmrb.wisc.edu/ref_info/statful.htm#1'

        html_request=requests.get(url)
        get_content=html_request.content

        soup=BeautifulSoup(get_content,'lxml')
        convert=soup.prettify()
        get_only_text=soup.get_text()
        convert_string_to_lines=get_only_text.split('\n')

        for i in range(len(convert_string_to_lines)):
               convert_string_to_lines[i]=convert_string_to_lines[i].split(',')
        ...
        temp_list_H=[]
        temp_list_rest=[]
        properly_formatted_list=[]
        counter=0
        for lines in convert_string_to_lines:
            joined=''.join(lines)
            if re.findall('^[A-Z]{3}',joined) != []:
                splitter=joined.split()
                del splitter[2:6]
                if splitter[1] == 'N' or splitter[1] == 'HA' or splitter[1] == 'HA2' or splitter[1] == 'HA3' or splitter[1] == 'C' or splitter[1] == 'CA' or splitter[1] == 'CB' or splitter[1] == 'H':
                    splitter[0]=acid_map[splitter[0]]
                    counter+=1
                    if splitter[1] == 'N':
                        temp_list_N.append(splitter)
                    elif splitter[1] == 'H':
                        temp_list_H.append(splitter)
                    elif splitter[0] == 'P':
                        temp_list_rest.append(splitter)
                        if counter<2:
                            counter+=1
                    else:
                        temp_list_rest.append(splitter)
                    ...
                    if counter == 6:
                        if splitter[0] == 'P':
                            compiled_list=temp_list_N+temp_list_rest+[['P','H','1000','1']]
                            properly_formatted_list.append(compiled_list)
                            counter=0
                            temp_list_N.clear()
                            temp_list_H.clear()
                            temp_list_rest.clear()
                        else:
                            compiled_list=temp_list_N+temp_list_rest+temp_list_H
                            properly_formatted_list.append(compiled_list)
                            counter=0
                            temp_list_N.clear()
                            temp_list_H.clear()
        ...
        flattened_list=[]
        for lines in properly_formatted_list:
            for flatten in lines:
                flattened_list.append(flatten)

        sparta_list=[]
        amino_acid_count=int(seq_start)-1
        os.chdir(seq_directory)
        with open(seq_file) as sequence:
            for lines in sequence:
                modifier=lines.strip()
                for amino_acids in modifier:
                    amino_acid_count+=1
                    for values in flattened_list:
                        if amino_acids == values[0]:
                            sparta_list.append(str(amino_acid_count)+(' '.join(values)))

        sparta_file_list3=[]
        for values in sparta_list:
            splitter1=values.split()
            splitter1[0:2]=[''.join(splitter1[0:2])]
            sparta_file_list3.append(' '.join(splitter1))
...
#This part compares the peaklist to the SPARTA file, and only appens amino acids that are in both SPARTA and the Peaklist file
        text_area.insert(tk.INSERT,'Converting Peaklist to match BMRB\n')
        text_area.update_idletasks()
        list3=[]
        count=0
        for lines in list5:
...
#Then it sums up these deviations (once you've done all 6 atoms), if the RMSD is above the set threshold, it prints it out
        sparta_file_list4=[]
        for lines in sparta_file_list3:
            modify=lines.strip()
            splitting5=modify.split()
            number_search=re.search('^-*\d+[A-Z]',splitting5[0])
            r=re.compile(number_search.group(0))
            comparison_to_sparta=list(filter(r.match,list3))
            if comparison_to_sparta != []:
                sparta_file_list4.append(modify)


        list4=[]
        number=0
        for experimental,predictions in zip(list3,sparta_file_list4):
            number+=1
...
#The compiled files can be useful to use for other SPARTA comparisons (such as determing unknowns), so they are saved for later use
        ...
        with open(save_file_sparta,'w') as file, open(save_file_peaklist,'w') as file2:
            for stuff_to_write in sparta_file_list4:
                file.write(stuff_to_write+'\n')
            for stuff_to_write2 in list3:
                    file2.write(stuff_to_write2+'\n')
        text_area.insert(tk.INSERT,'BMRB files written\n')
...
def nmrstarrun3():
    ...
    if nmrstarfile == ():
        ...
        with open(save_file_sparta,'w') as file, open(save_file_peaklist,'w') as file2:
            ...
            for stuff_to_write2 in list3:
                    file2.write(stuff_to_write2+'\n')
        text_area.insert(tk.INSERT,'Files written\n')
...
tk.Button(root,text='browse',command=NHSQC).grid(row=1,column=2)
tk.Button(root,text='browse',command=HNCA).grid(row=2,column=2)
tk.Button(root,text='browse',command=HNCACB).grid(row=3,column=2)
tk.Button(root,text='browse',command=HNCO).grid(row=4,column=2)
tk.Button(root,text='browse',command=save_file).grid(row=5,column=2)
tk.Button(root,text='browse',command=save_file2).grid(row=6,column=2)
tk.Button(root,text='enter',command=seq_number).grid(row=7,column=2)
tk.Button(root,text='enter',command=threshold).grid(row=8,column=2)
tk.Button(root,text='browse',command=nmrstar).grid(row=9,column=2)
...
tk.Button(root,text='Generate BMRB file only (for APS)',command=sparta_gen_only).grid(row=14,column=1)
